...
import com.isi.Mapper.ExcelMapper;
...
public class ExcelServiceImpl extends ServiceImpl<ExcelMapper, ProjectItem> implements ExcelService {
    ...
    @Override
    public Result getExcelFiled(MultipartFile file) {
    ...
    public Result readDataFromExcel(MultipartFile file) {
        ...
        for (int i = 1; i <= lastRowNum; i++) {
            ...
            //规避行数数据后几行为空
            if (rowList.size() < 100) {
                for (int k = 0; k < 100 - rowList.size(); k++) {
                    rowList.add("");
                }
            }

            // 添加数据,映射完能否导入数据，后期映射方案能不能使用，用来做内容替换方案待定
            ...
            projectItem.setContent(rowList.get(3).trim());
//            if ("干细胞".equals(rowList.get(3).trim())) {
//                projectItem.setType(1);
//            } else if ("白细胞".equals(rowList.get(3).trim())) {
//                projectItem.setType(2);
//            } else if ("红细胞".equals(rowList.get(3).trim())) {
